"""
Langfuse tracer implementation.
"""

from __future__ import annotations

import os
import uuid
from typing import Any, Dict, Optional
from .base import AbstractTracer, TraceContext


class LangfuseTracer(AbstractTracer):
    """Langfuse tracer implementation."""
    
    def __init__(self):
        super().__init__()
        try:
            from langfuse import Langfuse
            self.client = Langfuse()
            self._current_trace = None
            self._current_span = None
        except ImportError:
            raise ImportError("langfuse package not installed. Install with: pip install langfuse")
    
    def start_root_span(self, project_name: str, metadata: Optional[Dict[str, Any]] = None) -> TraceContext:
        """Start a root span using Langfuse trace."""
        try:
            # Redact PII from metadata
            safe_metadata = self._redact_pii(metadata or {})
            
            # Create root trace
            run_id = str(uuid.uuid4())
            self._current_trace = self.client.trace(name=project_name)
            
            context = TraceContext(
                run_id=run_id,
                span_id=run_id,
                metadata=safe_metadata
            )
            self.set_current_context(context)
            return context
            
        except Exception as e:
            import logging
            logging.warning(f"Failed to start Langfuse root span: {e}")
            # Fall back to noop behavior
            return super().start_root_span(project_name, metadata)
    
    def start_child_span(self, name: str, kind: str, metadata: Optional[Dict[str, Any]] = None) -> TraceContext:
        """Start a child span using Langfuse span."""
        try:
            if self._current_trace is None:
                # No parent trace, create a new root
                return self.start_root_span(name, metadata)
            
            # Redact PII from metadata
            safe_metadata = self._redact_pii(metadata or {})
            
            # Create child span
            span_id = str(uuid.uuid4())
            self._current_span = self._current_trace.span(
                name=name,
                input=safe_metadata
            )
            
            context = TraceContext(
                run_id=str(self._current_trace.id) if self._current_trace.id else span_id,
                span_id=span_id,
                metadata=safe_metadata
            )
            self.set_current_context(context)
            return context
            
        except Exception as e:
            import logging
            logging.warning(f"Failed to start Langfuse child span: {e}")
            # Fall back to noop behavior
            return super().start_child_span(name, kind, metadata)
    
    def end_span(self, context: TraceContext, outputs: Optional[Dict[str, Any]] = None) -> None:
        """End a span using Langfuse."""
        try:
            if self._current_span is not None:
                # Redact PII from outputs
                safe_outputs = self._redact_pii(outputs or {})
                self._current_span.end(output=safe_outputs)
                self._current_span = None
            elif self._current_trace is not None:
                # End the trace
                safe_outputs = self._redact_pii(outputs or {})
                self._current_trace.update(output=safe_outputs)
                self._current_trace = None
                
        except Exception as e:
            import logging
            logging.warning(f"Failed to end Langfuse span: {e}")
    
    def add_event(self, event_type: str, payload: Dict[str, Any], 
                  cost_usd: Optional[float] = None, tokens: Optional[int] = None) -> None:
        """Add an event to the current span."""
        try:
            if self._current_span is not None:
                # Redact PII from payload
                safe_payload = self._redact_pii(payload)
                
                # Add cost information if provided
                if cost_usd is not None:
                    safe_payload["cost_usd"] = cost_usd
                if tokens is not None:
                    safe_payload["tokens"] = tokens
                
                # Add event as observation
                self._current_span.observation(
                    name=f"event_{event_type}",
                    input=safe_payload
                )
                
        except Exception as e:
            import logging
            logging.warning(f"Failed to add Langfuse event: {e}")
    
    def get_trace_url(self) -> Optional[str]:
        """Get the trace URL for the current run."""
        try:
            if self._current_trace is not None and hasattr(self._current_trace, 'url'):
                return self._current_trace.url
        except Exception:
            pass
        return None 